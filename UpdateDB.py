...
def addAdmissionNote(admission_id, note_data, encryption_key):
    try:
        with hospitalDB.get_cursor() as cursor:
            sql = """
                INSERT INTO patientnote (
                    admission_id, author_id, note_type, note_text, note_datetime
                ) VALUES (
                    %s, %s, %s, pgp_sym_encrypt(%s, %s), %s
                )
            """
            cursor.execute(sql, (
                admission_id,
                note_data["author"],
                note_data["type"],
                note_data["text"],        # raw note text
                encryption_key,           # key used in pgp_sym_encrypt
                note_data["datetime"]
            ))
